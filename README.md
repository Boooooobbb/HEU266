...
- **多维度问卷建模**：围绕价值观、生活习惯、人格等维度进行信息采集（五模块问卷）
- **匹配与报告**：输出契合度、雷达图/描述等数据化浪漫的结果展示
- **慢社交机制**：限制匹配频次、固定开奖时间，降低社交压力
- **Supabase 后端**：Auth 认证、PostgreSQL 数据库（含 RLS 安全策略）、Edge Function 周期匹配与通知投递
...
> 最后更新：2026-04-27，详见 [docs/PROJECT_STATUS.md](docs/PROJECT_STATUS.md)
...
- ✅ **问卷系统**：五模块问卷页面与前端进度逻辑已落地，支持 Supabase 答案同步
- ✅ **匹配 Edge Function**：周批量匹配调度函数已编写（`docs/edge-functions/match-scheduler.ts`）
...
- 🚧 **匹配算法**：Edge Function 中的问卷相似度计算仍是占位实现，需补充可解释的评分函数
