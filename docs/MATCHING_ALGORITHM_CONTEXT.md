...
- **匹配频率**：每 7 天（每周一次）
- **揭晓时间**：固定每周三 12:00（北京时间，可配置）
- **生成时间**：固定每周三 11:55（北京时间）先生成并写入匹配结果，确保 12:00 准点可揭晓/通知（Cron 以 UTC 配置，见 docs/SUPABASE_SETUP.md）
- **配对数量**：每个用户每周最多 1 个潜在配对（MVP 先保证质量与闭环，后续可再扩展）
